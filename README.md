...
| 独立设置窗口 | 分为通用、外观、Touch Bar、实验和更新；调整语言、任务提示、颜色、不透明度、常驻行为与自动检查，提供实时浮窗预览。 |
| 自动联动宿主 | 首次运行后注册 LaunchAgent；ChatGPT / Codex 启动时自动运行，宿主完全退出后自动结束。 |
| 刷新与容错 | 额度定时刷新；短暂失败时保留上次数据，Token 旧数据用 `*` 标记，缺失数据用 `--` 显示。 |
| 自动安全检查 | 正式安装后默认按需检查正式版本；后台只做轻量提醒，下载安装仍须用户主动确认并通过完整校验。 |
...
面板稳定选择具有有效刘海区域的显示屏，以同一条连续黑色外壳从屏幕顶边包围物理刘海并延伸到摘要或详情底边；菜单栏高度内只覆盖左右辅助区域之间的中央范围，内容与交互仍从安全区下方开始，不占用两侧菜单。展开轮廓采用用户确认的短凹圆角、水平肩线和外圆角，收起与展开共享当前帧几何，避免底角露底、横向台阶和动画中途跳变。收起态同时展示任务状态、5 小时额度（或可用重置卡）和周额度；仅有周额度时只显示周额度，未知值显示 `--`。点击展开 Ledger 行式明细，查看重置/到期日期、Token 和点数；长内容自然增高，超出可用屏幕高度时可滚动，可刷新、切回桌面、打开设置或收起。任务变化不会自动展开；点击窗口外、切换应用或 Space 时收起。Esc 仅在本应用收到键盘事件时有效，不申请全局键盘监听权限。
...
几何、状态和布局已通过自动化模拟测试。仓库提供独立原生调试入口 `bash scripts/debug-notch-hud.sh`，它复用正式视图和几何，可切换 synthetic 屏幕、背景、内容宽度、展开状态和动画进度，并可生成截图；入口位于测试代码，不编入正式 App，也不会启动 Hook、额度请求或自动更新。详见[刘海连续外壳：原生模拟与验证](Documentation/NotchFusionDebug.md)。不同 Mac 型号的实体刘海接缝、透明角落点击穿透、全屏、自动隐藏菜单栏、锁屏/唤醒及多屏切换仍可能受系统版本和硬件布局影响；遇到问题可切换为桌面浮窗。
