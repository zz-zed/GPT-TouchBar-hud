...
在“设置 → 实验功能”打开 Hooks 配置，审阅计划后才可应用；仅打开窗口不会启用监测或写入配置。启用需宿主正常信任，新建或变更的 Hook 可能需要重新信任和重新打开任务。额度与 Token 数据来源不变。
...
面板稳定选择具有有效刘海区域的显示屏，从刘海下沿向下延伸，不在两侧菜单栏区域放置双翼。收起态同时展示任务状态、5 小时额度（或可用重置卡）和周额度；仅有周额度时只显示周额度，未知值显示 `--`。点击展开 Ledger 行式明细，查看重置/到期日期、Token 和点数；长内容自然增高，超出可用屏幕高度时可滚动，可刷新、切回桌面、打开设置或收起。任务变化不会自动展开；点击窗口外、切换应用或 Space 时收起。Esc 仅在本应用收到键盘事件时有效，不申请全局键盘监听权限。
