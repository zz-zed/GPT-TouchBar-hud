...
整体配图为功能与设备场景示意，使用演示数据；实际字形、间距及控件以原生界面为准。本文按当前源码说明，正式安装包可能尚未包含最新界面，体验当前效果可[从源码构建](#从源码构建)。当前正式版是 [v0.1.29 / Build 32](Documentation/RELEASE-0.1.29.md)：它已包含 Compact / Peek / Expanded 三态能力，但通用设置仍使用“刘海始终显示额度”复选框；当前源码将同一偏好改为“刘海常驻形态”两项菜单，旧选择会保留。
...
当前源码的“刘海常驻形态”提供“静止态 Compact（默认）”和“额度预览态 Peek”；Compact 在悬停时展示额度，Peek 在静止时也保留额度。v0.1.29 正式安装包使用“刘海始终显示额度”复选框表达同一选择：关闭对应 Compact，开启对应 Peek。两种界面复用同一个布尔偏好，不会重置旧选择。
...
- **Compact**：默认静止态，黑色外壳从屏幕顶边延伸到刘海两侧，显示应用与任务指示器。
- **Peek**：悬停 Compact 时横向展示额度和重置时间；也可选为常驻形态。
