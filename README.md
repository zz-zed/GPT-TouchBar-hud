...
整体配图为功能与设备场景示意，使用演示数据；实际字形、间距及控件以原生界面为准。当前正式版为 [v0.1.30 / Build 33](https://github.com/zz-zed/GPT-TouchBar-hud/releases/tag/v0.1.30)，提供“刘海常驻形态”两项选择，未保存偏好时默认 Peek，已有选择继续保留。详见[发布验证记录](Documentation/RELEASE-0.1.30.md)。
...
“设置 → 通用 → 刘海常驻形态”提供“静止态 Compact”和“额度预览态 Peek（默认）”；Compact 在悬停时展示额度，Peek 在静止时也保留额度。从 v0.1.29 升级时，原“刘海始终显示额度”复选框关闭对应 Compact、开启对应 Peek；已有选择继续保留，只有从未保存过该偏好时才默认 Peek。
