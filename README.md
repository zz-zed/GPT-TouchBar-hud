...
| 收起详情 | 保留刘海下方的双额度摘要，仅收起 Ledger 明细。 |
...
| 分页 | 设置内容 |
| --- | --- |
| 通用 | 选择显示形式、显示/隐藏状态、菜单栏内容和中文 / English，并开启或关闭任务状态提示。语言影响额度与任务信息，导航和设置保持中文。 |
...
在“设置 → 通用 → 显示模式”选择“刘海融合”或“自动”，再打开“显示状态面板”。已有用户仍默认桌面浮窗且不主动显示；显示形式和显示/隐藏状态分别保存。桌面浮窗颜色与透明度保持独立，刘海面板始终为黑色不透明外观。

面板跟随主显示屏，从刘海下沿向下延伸，不在两侧菜单栏区域放置双翼。收起态同时展示任务状态、5 小时额度（或可用重置卡）和周额度；仅有周额度时只显示周额度，未知值显示 `--`。点击展开 Ledger 行式明细，查看重置/到期日期、Token 和点数，可刷新、切回桌面、打开设置或收起。任务变化不会自动展开；点击窗口外、切换应用或 Space 时收起。Esc 仅在本应用收到键盘事件时有效，不申请全局键盘监听权限。

菜单栏支持“自动 / 仅图标 / 单额度 / 完整额度”。自动模式下，面板显示时仅留状态图标，隐藏后补回主额度；主动选择完整额度则一直保留。菜单和设置均能恢复隐藏的面板，再次双击正在运行的应用可打开设置。

无有效刘海几何或 macOS 11 时自动回退桌面浮窗，保留原显示形式偏好；接回刘海主屏后恢复。桌面浮窗显示、内容宽度改变和拖动结束时会校正可用屏幕边界。锁屏/休眠时面板暂时隐藏，恢复后仍尊重用户的可见性设置。Touch Bar 保持独立，没有 Touch Bar 的 Mac 同样支持菜单栏与浮窗。

几何、状态和布局已通过自动化模拟测试。不同 Mac 型号的实体刘海接缝、透明角落点击穿透、全屏、自动隐藏菜单栏、锁屏/唤醒及多屏切换仍可能受系统版本和硬件布局影响；遇到问题可切换为桌面浮窗。
