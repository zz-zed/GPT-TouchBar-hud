...
- **刘海融合**：静止时显示两侧指示器，悬停预览额度，点击展开“额度 / 活动 / 用量”；未设置模式时按屏幕自动适配。
...
| 收起详情 | 返回刘海紧凑状态；开启“刘海始终显示额度”后保留额度预览。 |
...
面板稳定选择具有有效刘海区域的显示屏。静止时，黑色外壳从屏幕顶边延伸到刘海两侧，分别显示应用和任务指示器；悬停后横向展开额度和重置时间，点击后向下展开详情，可切换“额度 / 活动 / 用量”。开启“刘海始终显示额度”后，静止时保留额度预览。详情中的长内容可滚动，底部提供刷新、设置、隐藏和收起。任务变化不会自动展开面板。
