...
final class PreferencesWindowController: NSWindowController {
    ...
    init(appearance: HUDAppearance) {
        ...
        let window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 490, height: 365), styleMask: [.titled, .closable], backing: .buffered, defer: false)
    ...
    private func configure() {
        ...
        NSLayoutConstraint.activate([tabs.leadingAnchor.constraint(equalTo: content.leadingAnchor, constant: 20), tabs.trailingAnchor.constraint(equalTo: content.trailingAnchor, constant: -20), tabs.topAnchor.constraint(equalTo: content.topAnchor, constant: 16), tabs.heightAnchor.constraint(equalToConstant: 225)])
        language.addItems(withTitles: ["中文", "English"])
        color.addItems(withTitles: HUDAppearance.ColorChoice.allCases.map(\.title))
        for control in [language, color] { control.target = self; control.action = #selector(changed(_:)) }
        for control in [tasks, persistent] { control.target = self; control.action = #selector(changed(_:)) }
        ...
        let general = column([row("信息语言", [language]), tasks, note("语言控制额度与任务信息。导航和设置保持中文。")])
        ...
        let caption = note("Quiet · 实时数据与外观预览")
