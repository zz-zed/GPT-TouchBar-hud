...
> 历史准备记录：下文描述发布前的本地候选。2026-09-21 已完成正式发布，当前状态与正式包校验见 [v0.1.29 发布记录](RELEASE-0.1.29.md)。
...
以 32 pt 刘海安全区为例，展开尺寸由 800 × 280 pt 改为 520 × 250 pt，矩形占用面积减少约 42%。顶部由直角改为 26 pt 连续曲线，底部由 14 pt 增至 28 pt；圆角随实际呈现高度形成，快速反向动画也沿用当前轮廓。绘制、裁剪与点击判断共用同一路径，四个透明角均可点击穿透。内容边距、光晕、阴影及顶部屏幕锚点均保留调整前的值，仅修改面板面积与轮廓边角。

正文保留两列额度与三个详情页，内边距保持原值。窄屏额外预留双行页脚高度，长内容可滚动；静止/悬停状态、启动自动识别和用户手动选择的保存规则保持原样。
