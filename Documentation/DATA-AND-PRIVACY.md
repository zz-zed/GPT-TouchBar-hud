...
新增任务通常在 10 秒内被发现，已发现任务每 2 秒检查新增日志；单个文件每次最多读取 256 KiB。日志格式不兼容、事件缺失或读取失败时显示未知，不影响额度读取。

运行中的任务连续 30 分钟没有新事件后才降级为未知，以兼顾长时间无日志的命令与遗留状态。静默不是结束证据，未知也不是失败判定。

执行中优先展示；本轮完成提示保留 30 秒，之后进入空闲。明确中止的任务直接进入空闲，不显示完成提示。全部空闲时隐藏角标与浮窗状态摘要，仍有未知任务时保留 `?`。
