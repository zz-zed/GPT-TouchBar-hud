...
> 后续已获远端发布准备授权：正常推送 release/0.1.28、创建草稿 [PR #1](https://github.com/zz-zed/GPT-TouchBar-hud/pull/1)，并验证双架构 CI。下文“未推送/本地 arm64-only/未执行 Intel CI”是本地准备阶段的事实，不代表后续状态；当前固定候选、CI 结果与两架构产物来源/哈希以 PR 的验证记录为准。仍未授权合并 main、推送版本标签或公开 Release。
